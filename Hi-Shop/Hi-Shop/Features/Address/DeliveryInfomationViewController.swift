...
protocol DeliveryInfomationViewControllerDelegate: AnyObject {
    func createSuccessAddress(_ delivery: Delivery)
}
...
class DeliveryInfomationViewController: BaseViewController {
    ...
    weak var delegate: DeliveryInfomationViewControllerDelegate?
    ...
    fileprivate lazy var fullNameTextField: TitleTextField = {
        ...
        textField.addTarget(self,
                            ...
                            for: .editingChanged)
        textField.text = UserManager.user?.fullName
        return textField
    }()
    ...
    @objc func tapOnSaveAddress() {
        ...
        if self.delivery.name.isEmpty {
            self.delivery.name = UserManager.user?.fullName ?? ""
        }
        
        if self.delivery.phone.isEmpty {
            self.delivery.phone = UserManager.user?.phone ?? ""
        }
        
        let params = delivery.toDictionary()
        let endPoint = UserEndPoint.createAddress(bodyParams: params)
        self.showLoading()
        APIService.request(endPoint: endPoint) { [weak self] (apiResponse) in
            ...
            self.navigationController?.popViewControllerWithHandler(completion: {
                self.delegate?.createSuccessAddress(self.delivery)
            })
        } onFailure: { (error) in
