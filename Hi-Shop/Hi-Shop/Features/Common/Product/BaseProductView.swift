...
protocol BaseProductViewDelegate: AnyObject {
...
class BaseProductView: BaseView {
    ...
    func configData(product: Product) {
        ...
        if product.priceSale != 0 {
            finalPriceLabel.text = product.priceSale.currencyFormat
            originalPriceLabel.text = product.price.currencyFormat
            ...
            let originalPriceAttr = NSAttributedString(string: "\(product.price.currencyFormat)", attributes: attributed)
            originalPriceLabel.attributedText = originalPriceAttr
        } else {
            finalPriceLabel.text = product.price.currencyFormat
