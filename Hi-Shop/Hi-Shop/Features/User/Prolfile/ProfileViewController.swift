...
class ProfileViewController: BaseViewController {
    ...
    @objc private func processLogout() {
        ...
        { (action) in
            self.showLoading()
                let endPoint = UserEndPoint.postFCMToken(params: ["FCMToken": ""])
    ...
    override func touchUpInLeftBarButtonItem() {
        self.navigationController?.popViewControllerWithHandler {
            self.delegate?.handleUpdateInfo()
        }
    }
...
extension ProfileViewController {
    ...
    private func bindingBirthday() {
        viewModel.birthday.asDriver().map {$0}
        ...
        DOBTextField.rx.controlEvent(.editingDidEnd)
            .bind{ [weak self] in
                ...
                if !self.isPressCancel {
                    self.viewModel.birthday.accept(self.datePicker.date.serverDateFormat)
                } else {
