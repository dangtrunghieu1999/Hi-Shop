...
class ProfileViewModel {
    ...
    lazy var birthday    = BehaviorRelay<String?>(value: nil)
    lazy var userProfile = BehaviorRelay<User?>(value: UserManager.user)
    ...
    init() {
        ...
        userProfile.bind { [weak self] profile in
            ...
            self?.birthday.accept(profile?.dayOfBirth)
    ...
    private func profileIsEdited() -> Bool {
        ...
        let birthday  = self.birthday.value  != user?.dayOfBirth ? self.birthday.value : nil
