...
extension HomeViewController {
    ...
    func requestAPIProducts() {
        ...
        let endPoint = ProductEndPoint.getAllProduct(paramter: params)
        if !isLoadMore {
    ...
    private func getAPINotification() {
        ...
        APIService.request(endPoint: endPoint) { [weak self] apiResponse in
            ...
            let notifications = json?.arrayValue.map { Notifications(json: $0)} ?? []
            CartManager.shared.totalNotifications = self.filterCheckIsSeenNoti(notifications: notifications)
