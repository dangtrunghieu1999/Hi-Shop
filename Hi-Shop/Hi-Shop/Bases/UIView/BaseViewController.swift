...
open class BaseViewController: UIViewController {
    ...
    private (set) lazy var notifiButton: BadgeButton = {
        let button = BadgeButton(type: .custom)
        button.frame = CGRect(x: 0, y: 0, width: 30, height: 30)
        button.setImage(ImageManager.notification, for: .normal)
        button.imageView?.contentMode = .scaleAspectFit
        button.badgeNumber = CartManager.shared.totalNotifications
    ...
    @objc func handleNotificationChange() {
        notifiButton.badgeNumber = CartManager.shared.totalNotifications
    }
